...
void init() 
{
  ...
  if(DIVSIZE <= 20)
  {
    printf("Too large initial compartments\n");
    exit(12);
  }
...
int choose_one_compartment() 
{
  int i;
  double totprop = 0.0, prop = 0.0, r;
  
  // if theta = 1.0 (environment E_X), the proportion of X molecules determines the reaction propensities within compartments
  // if theta = -1.0 (environment E_Y), the proportion of Y molecules determines the reaction propensities within compartments

  // based on the total propensity (the sum of each reaction propensity in each compartment (totprop)), we select a compartment
  for(i = 0; i < N; i++)
    totprop += (0.5*(theta + 1.0)*comp[i].X - 0.5*(theta - 1.0)* comp[i].Y) / (comp[i].X + comp[i].Y);

  r = totprop * randd();
  
  // in the selected compartment one reaction takes place based on the witin-compartment propensities (prop)
  for(i = 0; i < N; i++)
  {
    prop += (0.5*(theta + 1.0)*comp[i].X - 0.5*(theta - 1.0)* comp[i].Y) / (comp[i].X + comp[i].Y);
    if(prop > r)
      break;
  }
...
void Gillespie_time_approx()
{
  ...
  for(i = 0; i < N; i++)
  {
    ...
    mu[i][2] = ma  * comp[i].X;  // X --m--> Y (X--, Y++)
    mu[i][3] = comp[i].b * comp[i].R * comp[i].Y;  // R+Y --b--> 2Y (Y++)
    mu[i][4] = mb * comp[i].Y;  // Y --m--> X (Y--, X++)
  }
...
void Gillespie_step(int i) 
{
  ...
  mu[0] = comp[i].a * comp[i].R * comp[i].X; // R+X --a--> 2X (X++)
  mu[1] = g * comp[i].X * comp[i].Y;  // X+Y --g--> P (X--, Y--, P++)
  mu[2] = ma  * comp[i].X;  // X --m--> Y (X--, Y++)
  mu[3] = comp[i].b * comp[i].R * comp[i].Y;  // R+Y --b--> 2Y (Y++)
  mu[4] = mb * comp[i].Y;  // Y --m--> X (Y--, X++)
...
void statistics()
{
  ...
  printf("%d\t%lf\t%d\t%d\t%d\t%d\t%d\t%d\t%lf\n", sim, time, totx, toty, totp, xdom, ydom, numdiv, theta);
  fflush(stdout);
}
...
int main(void)
{
  ...
  for(sim = 0; sim < 11.0 * T; sim++)
  {
    i = choose_one_compartment();
    Gillespie_step(i);
    Gillespie_time_approx();
